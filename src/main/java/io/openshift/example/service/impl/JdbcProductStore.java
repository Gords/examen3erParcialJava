...
public class JdbcProductStore implements Store {
  ...
  private static final String INSERT = "INSERT INTO products (name, stock, discReader, storage, coreCount) VALUES (?, ?::BIGINT, ?, ?::BIGINT, ?::BIGINT)";

  private static final String SELECT_ONE = "SELECT * FROM products WHERE id = ?";

  private static final String SELECT_ALL = "SELECT * FROM products";

  private static final String UPDATE = "UPDATE products SET name = ?, stock = ?::BIGINT, discReader = ?, storage = ?::BIGINT, coreCount = ?::BIGINT WHERE id = ?";
  ...
  public Single<JsonObject> create(JsonObject item) {
    ...
    return db.rxGetConnection()
      ...
      .flatMap(conn -> {
        JsonArray params = new JsonArray().add(item.getValue("name")).add(item.getValue("stock", 0)).add(item.getValue("discReader","no")).add(item.getValue("storage", 0)).add(item.getValue("coreCount", 0));
        return conn
  ...
  public Completable update(long id, JsonObject item) {
    ...
    return db.rxGetConnection()
      .flatMapCompletable(conn -> {
        JsonArray params = new JsonArray().add(item.getValue("name")).add(item.getValue("stock", 0)).add(item.getValue("discReader","no")).add(item.getValue("storage", 0)).add(item.getValue("coreCount", 0)).add(id);
    ...
    return db.rxGetConnection()
      ...
      .map(array ->
        new JsonObject()
          ...
          .put("stock", array.getLong(2))
          .put("discReader",array.getString(3))
          .put("storage",array.getLong(4))
          .put("coreCount", array.getLong(5))
      );
  }
