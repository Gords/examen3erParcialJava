...
public class JdbcProductStore implements Store {
  ...
  public Single<JsonObject> create(JsonObject item) {
    ...
    return db.rxGetConnection()
      ...
      .flatMap(conn -> {
        JsonArray params = new JsonArray().add(item.getValue("name")).add(item.getValue("stock", 0)).add(item.getValue("discReader","no")).add(item.getValue("storage")).add(item.getValue("coreCount"));
        return conn
  ...
  public Completable update(long id, JsonObject item) {
    ...
    return db.rxGetConnection()
      .flatMapCompletable(conn -> {
        JsonArray params = new JsonArray().add(item.getValue("name")).add(item.getValue("stock", 0)).add(item.getValue("discReader","no")).add(item.getValue("storage")).add(item.getValue("coreCount")).add(id);
    ...
    return db.rxGetConnection()
      ...
      .map(array ->
        new JsonObject()
          ...
          .put("storage",array.getInteger(4))
          .put("coreCount", array.getInteger(5))
      );
  }
